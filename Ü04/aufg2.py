...
def proj():
    ...
    r_0 = np.zeros(len(cut_0))
    e_0 = np.zeros(len(cut_0))
    r_1 = np.zeros(len(cut_1))
    e_1 = np.zeros(len(cut_1))
    r_2 = np.zeros(len(cut_2))
    e_2 = np.zeros(len(cut_2))

    for i in range(len(cut_0)):
    ...
    ax_1.plot(cut_0,r_0,label="Reinheit")
    ax_1.plot(cut_0,e_0,label="Effizienz")
    ...
    fig1.savefig("./build/proj_aufg2.pdf")
    ...
    ax_6.set_xlabel(r"$x_{proj}$")
    ax_6.set_xlim(min_2,max_2)
    ax_6.set_ylim(0,1)
    ax_6.set_title("g(x)=-5/4x")
    fig2.savefig("./build/Unterscheidung_aufg2.pdf")
